...
* 面试：
	...
	* **算法**：
		* 矩阵快速幂问题？（1） n次方可以用多个组合子问题加速，如13就是`1*2*4（用2*2）*8（用4*4）`，转换为二进制问题，编程表示为 `while(N) {  if(N&1) res=res*A; n>>=1;  A=A*A; }`（2）递推式的求解：a2=a1+2a0，求解系数矩阵使得用a0,a1得到a1，a2的关系；（3）一个矩阵的m次方加速求解：综合上面两步进行拆分加速。参考文章：[递推求值](http://blog.csdn.net/lyhvoyage/article/details/22926265)
